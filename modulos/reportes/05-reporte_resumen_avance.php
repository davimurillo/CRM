...
<table class="table table-bordered" style="font-size:12px; width:100%" >
  ...
  <?php 
   $sql="SELECT tx_nombre_apellido,  (SELECT COUNT(id_oportunidad) FROM vie_seguimiento_control_oportunidad WHERE nu_tiempo<nu_tiempo_avance and id_usuario=a.id_usuario ) AS a_tiempo, (SELECT COUNT(id_oportunidad) FROM vie_seguimiento_control_oportunidad WHERE nu_tiempo>nu_tiempo_avance and id_usuario=a.id_usuario  ) AS a_destiempo, (SELECT COUNT(id_oportunidad) FROM vie_seguimiento_control_oportunidad WHERE id_usuario=a.id_usuario) as total FROM vie_seguimiento_control_oportunidad a WHERE 1=1";
  /*if ($_SESSION['rol']<3){
	  $sql.=" AND id_usuario=".$_SESSION['id_usuario'];
  }*/
  $sql.=" GROUP BY id_usuario, tx_nombre_apellido ORDER BY tx_nombre_apellido";
  $c=0;
  $total=0;
  $total_avanzada=0;
  $total_finalizadas=0;
